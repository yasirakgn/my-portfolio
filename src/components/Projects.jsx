  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {projectsData.map((p) => {
          // Construct base key for this item: projects.items.[id]
              ...
              <div className="p-8 flex flex-col flex-grow">
                ...
                <div className="space-y-4 mb-8 flex-grow">
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <p className="text-xs text-slate-500 mb-2 font-semibold uppercase tracking-wide">{t("projects.label.summary")}</p>
                    <ul className="text-xs text-slate-600 space-y-2">
                      <li className="flex gap-2"><span className="font-bold text-slate-700 min-w-[60px]">{t("projects.label.challenge")}</span> {t(`${k}.challenge`)}</li>
                      <li className="flex gap-2"><span className="font-bold text-slate-700 min-w-[60px]">{t("projects.label.solution")}</span> {t(`${k}.solution`)}</li>
                      <li className="flex gap-2"><span className="font-bold text-green-600 min-w-[60px]">{t("projects.label.impact")}</span> {t(`${k}.impact`)}</li>
                    </ul>
                  </div>
                </div>
                ...
                <footer className="mt-auto">
                  ...
                  <button className="w-full py-3 text-sm font-bold text-white bg-slate-900 rounded-xl group-hover:bg-blue-600 transition-colors flex items-center justify-center gap-2">
                    {t("projects.btn.detail")} <ArrowRight size={16} />
                  </button>
                </footer>
              </div>
            </article>
