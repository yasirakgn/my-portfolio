...
function SimulationPage() {
  ...
  const DEFAULT_PART_QUEUE_TEXT =
    "Mavi, Kırmızı, Yeşil, Mavi, Sarı, Kırmızı, Mor";
  ...
  const [message, setMessage] = useState(
    "Üretim planını düzenleyin ve başlamak için START'a basın."
  );
  const [partQueueText, setPartQueueText] = useState(
    DEFAULT_PART_QUEUE_TEXT
  );
  ...
  const initializeParts = useCallback(() => {
    ...
    if (invalidParts.length > 0) {
      const availableNames = Object.values(COLOR_MAP)
        .map((v) => v.name)
        .join(", ");
      setMessage(
        `🚨 Üretim planında GEÇERSİZ renkler: ${invalidParts.join(
          ", "
        )}. Sadece: ${availableNames}`
      );
    } else if (fullRecipe.length > 0) {
      setMessage(
        `Üretim planı yüklendi: ${fullRecipe.length} parça. Sistem hazır.`
      );
    } else {
  ...
  const plcStepsRef = useRef({
    0: () => {
      ...
      if (
        inputPartRef.current &&
        inputPartRef.current.x >= INPUT_PICKUP_X
      )
        setStep(10);
      else if (
        partRecipeRef.current.length === 0 &&
        outputPartsRef.current.every(
          (p) => !p.isMoving && p.counted
        )
      ) {
        setMessage("Tüm parçalar işlendi.");
        setIsRunning(false);
      }
    },
    10: () => {
      ...
      moveRobotTo(
        inputPartRef.current.x + PART_WIDTH / 2,
        inputPartRef.current.y + PART_HEIGHT / 2
      );
    ...
    30: () => {
      moveRobotTo(
        OUTPUT_DROP_X,
        MAIN_OUTPUT_CONVEYOR_Y - PART_HEIGHT / 2
      );
      if (distance(robotPosRef.current, targetRef.current) < 5)
        setStep(40);
    },
    40: () => {
      if (robotPartRef.current) {
        ...
        outputPartsRef.current.push({
          ...
          targetY: isTypeA
            ? MAIN_OUTPUT_CONVEYOR_Y - BRANCH_OFFSET - PART_HEIGHT
            : MAIN_OUTPUT_CONVEYOR_Y + BRANCH_OFFSET - PART_HEIGHT,
          diverterProgress: 0,
          counted: false,
        });
        ...
        robotPartRef.current = null;
        setMessage(
          `Parça (${partToDrop.name}) ana konveyöre bırakıldı.`
        );
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      if (container) {
        ...
        const newHeight =
          newWidth * (ORIGINAL_HEIGHT / ORIGINAL_WIDTH);
    ...
    const drawConveyors = () => {
      ...
      const drawConveyorLine = (startX, y, endX, label) => {
        ...
        for (let i = startX; i < endX; i += segmentLength) {
          const rollerX =
            i + ((animationRef.current.offset * 0.5) % segmentLength);
          ctx.beginPath();
        ...
        if (label) {
          ...
          ctx.textAlign = "left";
          ctx.font = "12px Inter";
          ctx.fillText(label, startX + 5, y - 5);
        }
      };
      ...
      drawConveyorLine(
        DIVERTER_X,
        MAIN_OUTPUT_CONVEYOR_Y - BRANCH_OFFSET,
        BIN_X + BIN_WIDTH,
        "Konveyör A"
      );
      drawConveyorLine(
      ...
      let angle = 0;
      const activePart = outputPartsRef.current.find(
        (p) => p.conveyorPhase === "DIVERTER_MOVE"
      );
      if (activePart) {
        const targetAngle =
          activePart.type === "TYPE_A" ? -Math.PI / 6 : Math.PI / 6;
      ...
      ctx.font = "12px Inter";
      ctx.fillText("Yönlendirici", DIVERTER_X, MAIN_OUTPUT_CONVEYOR_Y - 65);
    ...
    const drawParts = () => {
      ...
      if (rp) {
        ...
        rp.y =
          robotPosRef.current.y - rp.height / 2 - 15;
        drawPart(rp);
      }
    ...
    const drawRobot = () => {
      ...
      const dist = distance(currentPos, {
        x: ROBOT_BASE_X,
        y: ROBOT_BASE_Y,
      });
      ...
      const elbowX = midX + perpX * offsetMagnitude;
      const elbowY = midY + perpY * offsetMagnitude;
      ...
      const held = !!(
        robotPartRef.current && robotPartRef.current.held
      );
      ctx.fillStyle = held ? "#EF4444" : "#10B981";
      ctx.shadowColor = held
        ? "rgba(239, 68, 68, 0.8)"
        : "rgba(16, 185, 129, 0.8)";
      ctx.shadowBlur = 10;
      ctx.beginPath();
    ...
    const updateConveyorMovement = () => {
      ...
      outputPartsRef.current.forEach((p) => {
        ...
        if (p.conveyorPhase === "MAIN") {
          ...
          p.y =
            p.startY +
            (p.targetY - p.startY) * Math.min(p.diverterProgress, 1);
          ...
          if (p.x >= BIN_X) {
            ...
            if (!p.counted) {
              ...
              setMessage(
                `📦 Parça (${p.name}) Kutu ${p.conveyorId}'ye yerleştirildi.`
              );
            }
          }
        }
    ...
    const animate = () => {
      robotPosRef.current.x +=
        (targetRef.current.x - robotPosRef.current.x) * 0.1;
      robotPosRef.current.y +=
        (targetRef.current.y - robotPosRef.current.y) * 0.1;

      animationRef.current.offset =
        (animationRef.current.offset + CONVEYOR_SPEED) % 60;
    ...
    return () => {
      if (animationRef.current && animationRef.current.id)
  ...
  const handleStart = () => {
    ...
    setTimeout(() => {
      if (partRecipeRef.current.length === 0)
        return setMessage("🔴 HATA: Üretim planı boş veya geçersiz!");
      if (!isRunning) {
        setIsRunning(true);
        if (step !== 0) setStep(0);
        setMessage("Simülasyon BAŞLADI.");
      }
    }, 50);
  };
  ...
  const handleStop = () => {
    setIsRunning(false);
    setMessage("🔴 Simülasyon DURDURULDU.");
  };
  ...
  const StepDisplay = () => {
    ...
    if (
      ...
      description = "Tüm parçalar işlendi.";
    else if (!isRunning && step === 0) description = "Başlatılmayı bekliyor.";
    else {
      const map = {
        0: "Başlangıç: Yeni parça bekleniyor.",
        10: "Parçaya hareket.",
        20: "Parça kavranıyor.",
        30: "Bırakma noktasına hareket.",
        40: "Parça bırakılıyor.",
        50: "Ana pozisyona dönüş.",
      };
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-6">
      ...
      <div className="flex-1 lg:flex-shrink-0 lg:w-1/3 bg-gray-800 p-5 rounded-xl shadow-2xl border border-gray-700 space-y-5 flex flex-col">
        ...
        <div className="space-y-2">
          <p className="text-xs text-gray-400 italic">
            Virgülle ayırarak renkleri girin (Mavi, Kırmızı, Yeşil, Sarı,
            Mor).
          </p>
          <textarea
            value={partQueueText}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            ...
            className={`flex-1 w-full py-4 rounded-lg font-extrabold text-lg transition-all duration-300 flex items-center justify-center space-x-2 
              ${
                isRunning || validRecipeLength === 0
                  ? "bg-gray-600 text-gray-400 cursor-not-allowed shadow-inner"
                  : "bg-green-600 text-white shadow-lg shadow-green-900/50 hover:bg-green-700 hover:scale-[1.02]"
              }`}
          >
          ...
          <button
            ...
            className={`flex-1 w-full py-4 rounded-lg font-extrabold text-lg transition-all duration-300 flex items-center justify-center space-x-2 
              ${
                !isRunning
                  ? "bg-gray-600 text-gray-400 cursor-not-allowed shadow-inner"
                  : "bg-red-600 text-white shadow-lg shadow-red-900/50 hover:bg-red-700 hover:scale-[1.02]"
              }`}
          >
      ...
      >
        <h2 className="text-xl font-bold text-indigo-300 pb-2 border-b border-gray-700">
          CANVAS SİMÜLASYON
        </h2>
        ...
        <p
          className={`p-3 text-center rounded-lg font-semibold ${
            message.startsWith("🚨") || message.startsWith("🔴")
              ? "bg-red-700/50 text-red-300 border border-red-500"
